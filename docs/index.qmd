...
| Metal runtime | main app process / graphics processing unit (GPU) | Voxtral tensor (multi-dimensional array) operations through Metal, Apple's GPU programming interface |
| `mlx_lm.server` | child or reused local process | Gemma correction endpoint (request-serving network address) on loopback (the machine-internal network interface, 127.0.0.1) |
| browser client | iPhone/Android/personal computer (PC) browser | caption Server-Sent Events (SSE, a one-way HTTP stream the server keeps open to push updates) viewer or question form |
...
4. The decoder — the model stage that emits caption text — is primed with
   silence so startup work does not consume the first spoken words.
...
Assume the hardware provides frames at its native sample rate (audio
measurements per second) and channel layout.
...
serial inference queue (a queue that runs one submitted task at a time, giving
mutual exclusion). The mutable `vox_stream_t` is therefore never entered
concurrently. C transforms only newly stable positions and retains the exact
tails and key-value (KV) attention caches (stored per-position vectors the
model re-reads instead of recomputing) needed for the next chunk.

### Event transfer

Complete token pieces (a token is a model-chosen text fragment, often shorter
than a word) are drained from C and emitted through
...
`NWListener`, a random viewer capability (a secret bearer token: possession
alone grants access), and a Uniform Resource Locator (URL)
...
conformance in this application is backed by a serial queue, actor (a Swift
reference type whose mutable state is accessed by one task at a time), mutex,
or a combination whose responsibilities are documented in the relevant chapter.
